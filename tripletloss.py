...
def ranking_loss(labels, embeddings, margin, margin2, squared=False):
    ...
    #a,b,c,d=torch.where(mask==True)
   # a,b,c,d=a.cpu().numpy(),b.cpu().numpy(),c.cpu().numpy(),d.cpu().numpy()
   # for i in zip(a,b,c,d):
       #   print(i, end=' ')
    triplet_loss = mask.float() * triplet_loss
  #  print((triplet_loss <0).float().sum(),"损失值小于0的个数")
...
def _pairwise_distances(embeddings, squared=False):
    ...
    #distance是[样本数，样本数的]表示样本之间的L2距离
    if not squared:
...
def _get_triplet_mask(labels):
  #  print(labels)
    indices_equal = torch.eye(labels.size(0)).bool()#[16,16]
