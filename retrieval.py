...
def compute_mAP(img, vid, aud, txt):
	...
	i2all = mAP(image, np.vstack((image, text, audio, video)), image_labels,
				np.vstack((image_labels, text_labels, audio_labels, video_labels)))
	t2all = mAP(text, np.vstack((image, text, audio, video)), text_labels,
				np.vstack((image_labels, text_labels, audio_labels, video_labels)))
	a2all = mAP(audio, np.vstack((image, text, audio, video)), audio_labels,
				np.vstack((image_labels, text_labels, audio_labels, video_labels)))
	v2all = mAP(video, np.vstack((image, text, audio, video)), video_labels,
				np.vstack((image_labels, text_labels, audio_labels, video_labels)))
